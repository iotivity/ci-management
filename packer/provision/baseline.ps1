...
function fetch-zip
{
  ...
  if ( check-hash $abs_zipfile $expected_hash ){
    echo "hash match"
  }else{
...
function fetch-exe
{
  ...
  if ( check-hash $tmp_dir+$bin $expected_hash ){
    echo "hash match"
  }else{
...
fetch-zipexe "754_1_ssl" 'curl_{0}.zip' "curl.exe" 'http://www.paehl.com/open_source/?download={0}' $curl_hash
...
docurl "http://moonunit.colliertech.org/~cjac/tmp/$file"
      $download_dir+$file
#
# pywin32
#
